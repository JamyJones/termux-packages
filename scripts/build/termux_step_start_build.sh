termux_step_start_build() {
	...
	local PACKAGES_WANTING_VERSIONS_ALIGNED="$(grep -R -l -E 'TERMUX_PKG_ALIGN_VERSION_WITH=["'"'"']?'"$TERMUX_PKG_NAME"'["'"'"']?' --include="build.sh" ${TERMUX_SCRIPTDIR} ||:)"
	...
	for _build_sh in ${PACKAGES_WANTING_VERSIONS_ALIGNED}; do
		local version="$(set +e +u; . "${_build_sh}" >/dev/null 2>&1; echo ${TERMUX_PKG_VERSION#*:})"
